...
function unfilterResults(filter) {
    ...
    let listToFilter = groupsList.get(category);
    ...
    if (filterGroups.has(str)) {
        ...
        // also make sure to go through all of the lists which are filtered
        // without the caterogies and then update them

        displayResults();
    } else {
        // we haven't seen this combo yet so we have to get rid of this
        ...
        //let listToFilter = groupsList.get(category);
        let newItems = listToFilter;
        if (newItems.length === 0) {
            newItems = allResults;
        }

        // now get the list to filter, and then loop through all the filters
        // for this cateogry and build up the new filter list that way
        ...
        // now loop through all of the categories and fix the to be updated
        // go through all of the current filters per list and if its not the category
        // then you can filter on it
        for (let [key, value] of groupsList) {
            // we don't want to filter if the list is
            // the current category

            let newCategoryGroupList = allResults;
            // otherwise, we are going to loop through
            // all of the filter lists for each of the categories

            // if the filters for the current category are not this
            // same category we are building up the category list for then
            for (let [key2, value2] of categoricalFilters) {
                if (key2 !== key) {
                    let categoryFilters = categoricalFilters.get(key2);
                    let newItems = [];
                    for (let i = 0; i < categoryFilters.length; i++) {
                        let addedIn = filterOnList(newCategoryGroupList, categoryFilters[i], key2);
                        for (let j = 0; j < addedIn.length; j++) {
                            if (!newItems.includes(addedIn[j])) {
                                newItems.push(addedIn[j]);
                            }
                    ...
                    if (newItems.length !== 0) {
                        newCategoryGroupList = newItems;
                    }
                }
            }
            groupsList.set(key, newCategoryGroupList);
        }
    }
...
function filterCurrResults(filter) {
    ...
    // get the list that is associated with this filters category
    // note that this list is the list which contains all items
    // filtered on all other current filters *but* the current
    // filter clicked from this category
    // EXAMPLE: if you are filtering currently on french, keyboard,
    // ios, and harware and click spanish this list will be all items
    // which are ios, keyboard and hardware -- but with all languages
    // not just french.
    let listToFilter = groupsList.get(category);
